...
class GPSegmentation:
    ...
    def __init__(self, dim, nclass, MAX_LEN=20, MIN_LEN=5, AVE_LEN=10, SKIP_LEN=1):
        ...
        self.prior_table = [
            i * math.log(self.AVE_LEN) - self.AVE_LEN - sum(np.log(np.arange(1, i + 1)))
    ...
    def calc_emission_logprob(self, c, segm):
        ...
        if len(segm) > 2:
            ...
            log_plen = (
                slen * math.log(self.AVE_LEN) + (-self.AVE_LEN) * math.log(math.e)
            ) - (sum(np.log(np.arange(1, slen + 1))))
        ...
        for c in range(self.numclass):
            ...
            for k in range(self.MAX_LEN):
                for dim in range(self.dim):
                    mu = params[dim][0][k]
                    sig = params[dim][1][k]
                    emission_prob_all[c, k, 0 : T - k] += -math.log(
                        math.sqrt(2 * math.pi * sig**2)
                    ) - (d[k:, dim] - mu) ** 2 / (2 * sig**2)
    ...
    def save_model(self, basename):
        ...
        for n, segm in enumerate(self.segments):
            ...
            for i, s in enumerate(segm):
                ...
                cut_points[-1] = 1
            np.savetxt(
                basename / ("segm%03d.txt" % n),
                np.vstack([classes, cut_points]).T,
                fmt=str("%d"),
            )
        ...
        for c in range(self.numclass):
            np.save(
                basename / ("class%03d.npy" % c),
                np.array(self.segm_in_class[c], dtype=object),
            )
        ...
        for t in range(T):
            for k in range(self.MIN_LEN, self.MAX_LEN, self.SKIP_LEN):
                ...
                for c in range(self.numclass):
                    ...
                    if tt >= 0:
                        prev_prob = (
                            log_a[tt, :, :] + z[tt] + np.log(self.trans_prob[:, c])
                        )
    ...
    def forward_filtering(self, d):
        ...
        emission_prob_all = self.calc_emission_logprob_all(d)
        forward_prob = calc_forward_probability(
            emission_prob_all,
            self.trans_prob,
            self.trans_prob_bos,
            self.trans_prob_eos,
    ...
    def calc_trans_prob(self):
        ...
        # 正規化
        self.trans_prob = self.trans_prob / self.trans_prob.sum(1).reshape(
            self.numclass, 1
        )
